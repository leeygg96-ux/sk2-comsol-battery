/*
 * SOC100_3cell_offset_v03.java
 *
 * Three-cell thermal-abuse / propagation baseline derived from
 * SOC100_paper_revise_2 and SOC100_3cell_offset_v02.
 *
 * Main assumptions in v0.3
 ...
 *  - Hchem_base is already the volume-corrected total additional energy for
 *    ONE scaled cell, as specified by the model owner.
 ...
 *  - Thermal properties are assigned through explicit COMSOL Material nodes.
 ...
 *
 * IMPORTANT
 *  - Run with COMSOL 6.4 Java or compile it using the COMSOL classpath.
...
public class SOC100_3cell_offset_v03 {
  ...
  private static void addAnisotropicMaterial(
      Model model, String tag, String label, String selection,
      String kx, String ky, String kz, String rho, String cp) {
    model.component("comp1").material().create(tag, "Common");
    model.component("comp1").material(tag).label(label);
    model.component("comp1").material(tag).selection().named(selection);
    model.component("comp1").material(tag).propertyGroup("def")
        .set("thermalconductivity", diagonalTensor(kx, ky, kz));
    model.component("comp1").material(tag).propertyGroup("def")
        .set("density", new String[]{rho});
    model.component("comp1").material(tag).propertyGroup("def")
        .set("heatcapacity", new String[]{cp});
  ...
  private static void trySetLowerLimit(Model model, String solverVariableTag, String limit) {
    String[] candidateKeys = new String[]{"lowerlimit", "lower", "min"};
    for (String key : candidateKeys) {
      try {
        model.sol("sol1").feature("v1").feature(solverVariableTag).set(key, limit);
        System.out.println("Applied solver lower-limit candidate " + key + "=" + limit
            + " for " + solverVariableTag);
        return;
      } catch (RuntimeException ex) {
        /* COMSOL version/export variants differ here; local 6.4 build must confirm. */
      }
    }
    System.out.println("Lower-limit API was not available for " + solverVariableTag
        + "; verify in COMSOL 6.4 if bounds are required by the exported solver tree.");
  }

  private static void addTransientSolver(Model model) {
    ...
    model.sol("sol1").feature("t1").set("tlist", "range(0,0.1,500)");
    ...
    model.sol("sol1").feature("t1").feature("se1").label("Temperature / reaction segregated loop");

    model.sol("sol1").feature("t1").feature("se1").create("ssT", "SegregatedStep");
    model.sol("sol1").feature("t1").feature("se1").feature("ssT")
        .set("segvar", new String[]{"comp1_T"});

    model.sol("sol1").feature("t1").feature("se1").create("ssTR", "SegregatedStep");
    model.sol("sol1").feature("t1").feature("se1").feature("ssTR").set("segvar",
        new String[]{"comp1_c_sep", "comp1_Qaccum", "comp1_c_e", "comp1_c_neg",
            "comp1_c_sei", "comp1_c_pos1", "comp1_c_pos2"});

    model.sol("sol1").feature("t1").create("dT", "Direct");
    model.sol("sol1").feature("t1").feature("dT").label("PARDISO, temperature step");
    model.sol("sol1").feature("t1").feature("dT").set("linsolver", "pardiso");
    model.sol("sol1").feature("t1").feature("se1").feature("ssT").set("linsolver", "dT");

    model.sol("sol1").feature("t1").create("dTR", "Direct");
    model.sol("sol1").feature("t1").feature("dTR").label("PARDISO, reaction-state step");
    model.sol("sol1").feature("t1").feature("dTR").set("linsolver", "pardiso");
    model.sol("sol1").feature("t1").feature("se1").feature("ssTR").set("linsolver", "dTR");

    model.sol("sol1").attach("std1");

    trySetLowerLimit(model, "comp1_c_sep", "0");
    trySetLowerLimit(model, "comp1_c_e", "0");
    trySetLowerLimit(model, "comp1_c_neg", "0");
    trySetLowerLimit(model, "comp1_c_sei", "0");
    trySetLowerLimit(model, "comp1_c_pos1", "0");
    trySetLowerLimit(model, "comp1_c_pos2", "0");
    trySetLowerLimit(model, "comp1_Qaccum", "0[J]");
  }

  public static Model buildModel() {
    Model model = ModelUtil.create("Model");
    model.label("SOC100_3cell_offset_v03.mph");
    ...
    /* ------------------------------------------------------------------ */
    model.component("comp1").selection().create("shellAll", "Union");
    model.component("comp1").selection("shellAll").set("input",
        new String[]{"geom1_shell1_dom", "geom1_shell2_dom", "geom1_shell3_dom"});
    model.component("comp1").selection("shellAll").label("All pouch shell domains");

    model.component("comp1").selection().create("cell1Active", "Union");
    model.component("comp1").selection("cell1Active").set("input", new String[]{"geom1_act1_dom"});
    model.component("comp1").selection("cell1Active").label("Cell 1 active domain");

    model.component("comp1").selection().create("cell2Active", "Union");
    model.component("comp1").selection("cell2Active").set("input", new String[]{"geom1_act2_dom"});
    model.component("comp1").selection("cell2Active").label("Cell 2 active domain");

    model.component("comp1").selection().create("cell3Active", "Union");
    model.component("comp1").selection("cell3Active").set("input", new String[]{"geom1_act3_dom"});
    model.component("comp1").selection("cell3Active").label("Cell 3 active domain");

    model.component("comp1").selection().create("activeAll", "Union");
    model.component("comp1").selection("activeAll").set("input",
        new String[]{"cell1Active", "cell2Active", "cell3Active"});
    ...
    model.component("comp1").selection().create("heaterDomain", "Union");
    model.component("comp1").selection("heaterDomain").set("input", new String[]{"geom1_heater1_dom"});
    model.component("comp1").selection("heaterDomain").label("External heater proxy domain");
    ...
    /* 6. Material nodes                                                   */
    /* ------------------------------------------------------------------ */
    addIsotropicMaterial(model, "matShell", "Pouch shell", "shellAll",
        "0.26[W/(m*K)]", "1150[kg/m^3]", "1700[J/(kg*K)]");
    addAnisotropicMaterial(model, "matActive", "Active jelly roll", "activeAll",
        "k_active_x", "k_active_y", "k_active_z", "rho_active", "Cp_active");
    addIsotropicMaterial(model, "matInterface", "Cell-to-cell interface layer", "interfaceAll",
        "k_int", "1000[kg/m^3]", "1000[J/(kg*K)]");
    addIsotropicMaterial(model, "matCopper", "Copper negative tab metal", "negativeMetal",
        "400[W/(m*K)]", "8960[kg/m^3]", "385[J/(kg*K)]");
    addIsotropicMaterial(model, "matAluminum", "Aluminum positive tab metal", "positiveMetal",
        "238[W/(m*K)]", "2700[kg/m^3]", "900[J/(kg*K)]");
    addIsotropicMaterial(model, "matHeater", "Heater proxy", "heaterDomain",
        "400[W/(m*K)]", "1[kg/m^3]", "1[J/(kg*K)]");

    /* ------------------------------------------------------------------ */
    /* 7. Local variables and thermal-runaway sources                      */
    /* ------------------------------------------------------------------ */
    ...
    model.component("comp1").physics("ht").feature("hsHeater").selection().named("heaterDomain");
    ...
    /* ------------------------------------------------------------------ */
    /* 9. Domain ODEs: independent fields over each disconnected cell      */
    /* ------------------------------------------------------------------ */
    ...
    /* ------------------------------------------------------------------ */
    /* 10. Mesh, study, solver, cell-wise probes                           */
    /* ------------------------------------------------------------------ */
